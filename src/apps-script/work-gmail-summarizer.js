/**
 * Gmail + Perplexity Integration - Work Email Summarizer
 * 
 * This Google Apps Script fetches unread emails from your work Gmail inbox
...
function doGet() {
  try {
    // Get unread emails from work inbox
    const emails = getUnreadEmails('work');
...
function formatEmailsForAI(emails) {
  if (!emails || emails.length === 0) {
    return '## 🔥 No Unread Emails Found\n\nYour work inbox is clean! 🎉';
  }
  
  let output = `## 🔥 Work Email Summary (${emails.length} emails)\n\n`;
  ...
  output += '\n> 🤖 Summarize the above work email dump and highlight action items, deadlines, and important links.';
...
function testScript() {
  try {
    const emails = getUnreadEmails('work');
    console.log(`Found ${emails.length} unread work emails`);
